...
void Fire::init() {
    ...
    for (int i = 0; i < FB_WIDTH; i++) {
        // 'height - 2' offset avoids line at the top of (my) display panel -
        // doesn't appear in simulator (??)
        fire_fb[(FB_HEIGHT - 2) * FB_WIDTH + i] = 0xf;
    }
}
...
void Fire::draw(Canvas &canvas) {
    ...
    for (int y = 0; y < FB_HEIGHT - 1; y++) { // 'height - 1' for display panel
...
#ifdef FIRE_LORES
            // canvas.point(x * FB_SCALE, y * FB_SCALE); // scanline-type effect
            // canvas.drawRect(x * FB_SCALE, y * FB_SCALE, 3, 3); // scanline-type effect
